...
from concurrent.futures import ThreadPoolExecutor, TimeoutError as FutureTimeoutError  # pylint: disable=no-name-in-module
import contextlib
from typing import (
    Dict,
    List,
...
rnode_logback_file = f'{rnode_directory}/logback.xml'
...
class Node:
    ...
    def get_rnode_address(self) -> str:
        ...
        # More flexible regex that matches any host (container name, IP, or hostname)
        # The address format is: rnode://<node-id>@<host>?protocol=<port>&discovery=<port>
    ...
    def _exec_run_with_timeout(self, cmd: Tuple[str, ...], stderr: bool = True) -> Tuple[int, str]:
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                # Note: The thread will continue running in the background, but we've timed out
                # The executor will clean up when it goes out of scope
    ...
    def cat_forward_file(self, public_key: str) -> str:
        return self.shell_out('cat', f'/opt/docker/forward_{public_key}.rho')

    def cat_bond_file(self, public_key: str) -> str:
        return self.shell_out('cat', f'/opt/docker/bond_{public_key}.rho')
...
class DeployThread(threading.Thread):
    def __init__(self, name: str, node: Node, contract: str, count: int, private_key: PrivateKey) -> None:  # pylint: disable=too-many-positional-arguments
...
def make_container_command(container_command: str, container_command_flags: AbstractSet,
    ...
    result = f'{container_command} {flags} {" ".join(opts)}'
    return result


def make_node( # pylint: disable=too-many-locals,too-many-arguments
    *,
    ...
    if wallets_file is not None:
        volumes.append(f'{wallets_file}:{rnode_wallets_file}')
    if extra_volumes:
...
def make_bootstrap_node(  # pylint: disable=too-many-arguments
    *,
    docker_client: DockerClient,
    network: str,
    bonds_file: str,
...
def make_peer_name(network_name: str, name: str) -> str:
    if name.isdigit():
        actual_name = f'peer{name}'
    else:
        actual_name = name
    return make_container_name(network_name=network_name, name=actual_name)


def make_peer(  # pylint: disable=too-many-arguments
    *,
    docker_client: DockerClient,
    network: str,
    name: str,
...
def started_peer(  # pylint: disable=too-many-arguments
    *,
...
@contextlib.contextmanager
def started_bootstrap_with_network(  # pylint: disable=too-many-positional-arguments
    context: TestingContext,
    cli_flags: Optional[AbstractSet] = None,
    cli_options: Optional[Dict] = None,
    synchrony_constraint_threshold: float = 0.0,
