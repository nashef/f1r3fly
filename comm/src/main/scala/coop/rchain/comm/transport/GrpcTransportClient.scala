    ...
    for {
      ...
      r <- if (c.grpcTransport.isTerminated)
            Sync[F].delay(c.buferSubscriber.cancel()) >>
              Log[F].debug(
                s"Channel to peer ${peer.toAddress} is terminated and in ${c.grpcTransport.getState(false)} state; removing from connections map"
              ) >>
              channelsMap.update(_ - peer) >> getChannel(peer)
          else c.pure[F]
    } yield r

  private def withClient[A: Show](peer: PeerNode, timeout: FiniteDuration)(
